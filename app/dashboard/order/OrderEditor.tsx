...
export default function OrderEditor({
  ...
  const sectionTitles = useMemo(() => {
    ...
    const others = arr.filter(
      (a) => !priority.some((pr) => normalize(pr) === normalize(a))
    );
  ...
  const editLine = (line: LineItem) => {
    ...
    setLineStatus(line.lineStatus || "processing");

    const variantPrice = Number(
      catById
        .get(line.categoryId)
        ?.variants?.find((v) => v.id === line.variantId)?.unitPrice ?? 0
    );
  ...
  const storItems = items.filter((i) =>
    isStorPerdeByName(catById.get(i.categoryId)?.name || "")
  );
  const aksesuarItems = items.filter(
    (i) => normalize(catById.get(i.categoryId)?.name || "") === "AKSESUAR"
  );
    ...
    <div className="mx-auto my-4 bg-white text-black print:my-0 print:bg-white print:text-black">
      ...
      <div className=" m-auto w-[210mm] print:w-auto print:min-h-[auto] flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">
          {orderType == 0 ? "Yeni Sipariş" : "Sipariş Teklifi"}
        </h1>
        <div className="flex items-center gap-2">
          {profile.logoUrl ? (
            ...
            <img
              src={profile.logoUrl}
              alt="Logo"
              className="h-8 w-8 object-contain rounded-md ring-1 ring-gray-200"
            />
          ) : null}
      ...
      <div className="m-auto w-[210mm] min-h-[297mm] print:w-auto print:min-h-[auto]">
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <div className="col-span-1">
            ...
            <textarea
              className="input mt-1 min-h-[227px] w-full"
              value={orderNote}
              ...
              <div>
                ...
                <select
                  ...
                  title={
                    isStorPerdeByName(selectedCategory?.name || "")
                      ? "STOR PERDE m² hesabı: file dikkate alınmaz"
                      : undefined
                  }
                >
                ...
                {isStorPerdeByName(selectedCategory?.name || "") && (
                  <div className="mt-1 text-[10px] text-emerald-700 font-medium">
                    STOR PERDE <b>m²</b> hesabı ile (En×Boy/10.000) fiyatlanır;
                    file dikkate alınmaz.
                  </div>
                )}
              </div>
              <div>
            ...
            try {
              ...
              if ((Number(paidAmount) || 0) > 0) {
                const payRes = await fetch(
                  `/api/orders/${created.id}/payments`,
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    credentials: "include",
                ...
                if (!payRes.ok) {
                  console.error(
                    "POST /api/orders/:id/payments failed:",
                    await payRes.text().catch(() => "")
                  );
                  toast.error(
                    "Sipariş oluşturuldu, ancak ödeme kaydı eklenemedi. Ödemeyi sipariş detayından ekleyebilirsiniz."
                  );
                }
              }
            ...
            >
              {!it ? (
                ...
                <div className="text-[9px] leading-3">
                  ...
                  <div className="font-medium pr-24">
                    <b>Tür :</b> {variantName}
                  </div>
                  <div>
                    ...
                    {it.height} cm
                    <br />
                    <b>File :</b> {it.fileDensity}x · <b>Birim :</b>{" "}
                  ...
                  <div className="absolute right-1 bottom-1 flex opacity-0 transition group-hover:opacity-100 print:hidden">
                    ...
                    <button
                      className="ml-1 px-1 py-0.5 text-[10px] border bg-white"
...
function SectionQuickPlus({
  ...
  const visibleRows = Array.from({ length: order.length }).map(
          ...
          if (it) {
            ...
            label = (
              <>
                ...
                {isStor && <> • Alan: {fmt(area)} m²</>} • {fmt(it.subtotal)}
              </>
            );
          }
  ...
  return (
    <div className="text-sm space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="font-semibold">{fmt(grandTotal)}</div>
      </div>
