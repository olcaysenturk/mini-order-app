...
export default function OrdersCalendarPage() {
  ...
  const totals = useMemo(() => {
    ...
    for (const o of filteredOrders) {
      ...
      if (o.status !== "completed" && o.status !== "cancelled") inProgress += 1;
      const delivery = new Date(o.deliveryAt || o.deliveryDate || o.createdAt);
      delivery.setHours(0, 0, 0, 0);
      if (delivery >= today) upcomingCount += 1;
    }
    return { total, completed, inProgress, upcomingCount };
  }, [filteredOrders]);
  ...
  const summaryCards: Array<{ label: string; value: string; hint?: string }> = [
    { label: "Filtrelenen Sipariş", value: fmtInt(totals.total), hint: headerFilterLabelMap[headerFilter] },
    { label: "Tamamlanan", value: fmtInt(totals.completed), hint: "Seçili aralıkta tamamlanan kayıtlar" },
    { label: "Devam Eden", value: fmtInt(totals.inProgress), hint: "Takip edilen siparişler" },
    { label: "Yaklaşan Teslim", value: fmtInt(totals.upcomingCount), hint: "Bugünden itibaren planlanan teslimler" },
  ];
    ...
    <div className="flex flex-col bg-[linear-gradient(180deg,#f7f7fb_0%,#ffffff_20%)]">
      ...
      <header className="sticky top-0 z-20 border-b border-neutral-200/70 bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-4">
            ...
            <div className="relative z-10 grid gap-6 px-6 py-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] lg:items-center">
              <div className="space-y-5">
                ...
                <p className="max-w-2xl text-sm text-neutral-600">
                  Teslim tarihlerini, atölye aşamalarını ve tamamlanan siparişleri kolayca planlayın. Filtreler ve arama sonuçları yukarıdaki finansal özetlere yansır.
                </p>
          ...
          <div className="mt-3 grid gap-2 rounded-2xl border border-neutral-200 bg-white/90 p-3 shadow-sm sm:grid-cols-3 sm:gap-3">
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center justify-end gap-2">
                    ...
                    {/* <a
                      href={`/dashboard/orders/${o.id}?pay=1`}
