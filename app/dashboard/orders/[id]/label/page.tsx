...
export default function OrderLabelsThermal() {
  ...
  const orderTail = (order.id || '').slice(-6);
  const summaryCards = [
    { label: 'İşlemde Satır', value: `${filteredCount}/${totalCount}`, hint: 'Yalnızca işlemde olan kalemler etiketlenir.' },
    { label: 'Etiket Sayısı', value: labelCount, hint: 'Her satır için tek bir barkod etiketi üretilir.' },
    { label: 'Sipariş Kodu', value: `#${orderTail || order.id}`, hint: 'Sipariş ID’sinin son 6 hanesi.' },
    { label: 'Not Durumu', value: showNotes ? 'Notlar gösteriliyor' : 'Notlar gizli', hint: 'Kart üstünden yönetebilirsiniz.' },
  ];
              ...
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-neutral-600 ring-1 ring-neutral-200">
                    Sipariş #{orderTail || order.id}
                  </span>
                </div>
                <p className="max-w-2xl text-sm text-neutral-600">
                  Etiketler yalnızca <strong>İşlemde</strong> durumundaki satırlardan oluşturulur. Atölyeye aktar ve yazdır
                  aksiyonu tüm etiketleri tek seferde üretir.
                </p>
