...
export default function OrdersPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const o of filtered) {
      ...
      if (o.status !== "completed" && o.status !== "cancelled") activeCount += 1;
    }
    return { total, completed, activeCount, cancelled };
  }, [filtered]);
  const headerFilterLabelMap: Record<HeaderFilter, string> = {
    all: "Tüm teklifler",
    completed: "Onaylanan teklifler",
    active: "Aktif teklifler",
  };
  const summaryCards: Array<{ label: string; value: string; hint?: string }> = [
    { label: "Filtrelenen Teklif", value: fmtInt(stats.total), hint: headerFilterLabelMap[headerFilter] },
    { label: "Onaylanan", value: fmtInt(stats.completed), hint: "Tamamlanan teklifler" },
    { label: "Devam Eden", value: fmtInt(stats.activeCount), hint: "Bekleyen müşteri yanıtı" },
    { label: "İptal Edilen", value: fmtInt(stats.cancelled), hint: "İptal edilen teklifler" },
  ];
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-neutral-600">
                  Müşterilere ilettiğiniz teklifleri takip edin, durumlarını güncelleyin ve ekip koordinasyonunu kolaylaştırın.
                </p>
              </div>
