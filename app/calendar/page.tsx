...
function orderDayKey(o: Order) {
  return dateKeyFromISO(o.deliveryAt) ?? dateKeyFromISO(o.deliveryDate) ?? dateKeyFromISO(o.createdAt)!;
}
function startOfMonth(d: Date) { return new Date(d.getFullYear(), d.getMonth(), 1); }
function addMonths(d: Date, delta: number) { return new Date(d.getFullYear(), d.getMonth() + delta, 1); }
function isTodayYMD(ymd: string) { return ymd === toYMD(new Date()); }
...
function buildMonthGrid(currentMonth: Date): DayCell[] {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    const ymd = toYMD(d);
    const inCurrentMonth = d.getFullYear() === start.getFullYear() && d.getMonth() === start.getMonth();
...
export default function OrdersCalendarPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      if (dealerFilter !== "all" && (o.dealer?.name || "") !== dealerFilter) return false;
      if (needle) {
        const inHeader =
  ...
  const activeChips = useMemo(() => {
    ...
    if (statusFilter !== "all") chips.push({ key: "status", label: statusLabel[statusFilter], onClear: () => setStatusFilter("all") });
    if (dealerFilter !== "all") chips.push({ key: "dealer", label: `Bayi: ${dealerFilter}`, onClear: () => setDealerFilter("all") });
    if (q.trim()) chips.push({ key: "q", label: `Ara: “${q.trim()}”`, onClear: () => setQ("") });
    return chips;
  }, [statusFilter, dealerFilter, q]);

  return (
    <div className="flex h-dvh flex-col bg-[linear-gradient(180deg,#f7f7fb_0%,#ffffff_20%)]">
      {/* Sticky Header */}
        ...
        <div className="mx-auto max-w-7xl px-3 py-2 sm:px-6 sm:py-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2">
              <div className="inline-flex size-9 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-sm">
                <svg viewBox="0 0 24 24" className="size-5" aria-hidden>
                  <path fill="currentColor" d="M7 3h10a2 2 0 0 1 2 2v3H5V5a2 2 0 0 1 2-2zm14 8H3v8a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2zM7 14h4v4H7z" />
                </svg>
              </div>
              <div className="min-w-0">
                <h1 className="truncate text-lg font-bold tracking-tight sm:text-2xl">Takvim</h1>
                <div className="text-xs text-neutral-500 sm:hidden">{monthLabel}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden items-center gap-2 rounded-xl border border-neutral-200 bg-white p-1 shadow-sm sm:inline-flex">
                ...
                <div className="min-w-40 px-2 text-sm font-semibold">{monthLabel}</div>
                <button
                  className="inline-flex size-8 items-center justify-center rounded-lg hover:bg-neutral-50"
                  onClick={() => setCursor((c) => addMonths(c, +1))}
                  title="Sonraki Ay"
          ...
          {activeChips.length > 0 && (
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {activeChips.map((c) => (
                <span key={c.key} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-700 ring-1 ring-inset ring-neutral-200">
                  {c.label}
                  <button className="rounded p-0.5 hover:bg-neutral-200" onClick={c.onClear} aria-label="Temizle">
              ...
              <button
                ...
                onClick={() => { setQ(""); setStatusFilter("all"); setDealerFilter("all"); }}
              >
                Hepsini sıfırla
              </button>
            </div>
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-1 overflow-hidden">
          ...
          <div className="min-w-[720px] px-3 sm:px-6">
            ...
            <div className="grid grid-cols-7 gap-2 pt-2 sm:pt-4">
              {["Pzt", "Sal", "Çar", "Per", "Cum", "Cmt", "Paz"].map((d) => (
                <div key={d} className="px-2 py-1 text-center text-[11px] font-semibold text-neutral-500 sm:text-xs">{d}</div>
              ))}
            </div>

            {/* 6x7 grid */}
            <div className="grid grid-cols-7 gap-2 pb-3 sm:pb-4">
              {buildMonthGrid(cursor).map((c) => {
                ...
                const isOverdue =
                  list.some((o) => {
                    ...
                    const notDone = o.status !== "completed" && o.status !== "cancelled";
                    return notDone && dAt < new Date(today.getFullYear(), today.getMonth(), today.getDate());
                  }) && c.inCurrentMonth;
                const ring = c.isToday ? "ring-2 ring-indigo-400" : isOverdue ? "ring-2 ring-rose-300" : "";
                ...
                return (
                  ...
                  >
                    <div className="mb-1 flex items-center justify-between">
                      <span className={["inline-flex size-6 items-center justify-center rounded-full text-xs font-semibold sm:size-7",
                        c.isToday ? "bg-indigo-600 text-white" : "text-neutral-700"].join(" ")}>
                        {day}
                      </span>
                      {cnt > 0 && (
                        <span className="rounded-md bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium text-neutral-700 ring-1 ring-inset ring-neutral-200">{cnt}</span>
                      )}
                    </div>
                    ...
                    <div className="flex h-[calc(100%-28px)] sm:h-[calc(100%-30px)] flex-col gap-1 overflow-y-auto pr-1">
                      {preview.map((o) => (
                        ...
                        >
                          <span className={`inline-block h-2 w-2 rounded-full ${statusDot[o.status]}`} />
                          <span className="truncate">{o.customerName || "—"}</span>
                        </div>
                      ))}
                      {cnt > preview.length && (
                        <div className="text-[10px] text-neutral-500 sm:text-[11px]">+{cnt - preview.length} diğer</div>
                      )}
                    </div>
                  </button>
                );
            ...
            {/* Hata/Yükleniyor */}
            {loading && <div className="py-8 text-center text-sm text-neutral-500">Yükleniyor…</div>}
            {err && (
              <div className="mb-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">
                {err} <button className="underline underline-offset-2" onClick={() => setHeaderFilter((s) => s)}>Tekrar dene</button>
              </div>
            )}
          </div>
        </div>
        ...
        <aside
          className={[
            "absolute right-0 top-0 z-30 h-full w-full max-w-md transform border-l border-neutral-200 bg-white shadow-xl transition-transform duration-200 ease-out",
            selectedYMD ? "translate-x-0" : "translate-x-full",
          ].join(" ")}
          role="dialog"
          aria-modal="true"
          aria-label="Gün detay paneli"
        >
          <div className="flex items-start justify-between border-b border-neutral-200 p-3 sm:p-4">
            <div>
              <div className="text-[11px] text-neutral-500 sm:text-xs">Seçili Gün</div>
              <div className="text-base font-semibold sm:text-lg">
                {selectedYMD
                  ? new Intl.DateTimeFormat("tr-TR", { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date(selectedYMD))
                  : "—"}
              </div>
            </div>
            <button
            ...
            {!selectedYMD ? (
              ...
              <ul className="space-y-3">
                {dayMap.get(selectedYMD)!.map((o) => {
                  ...
                  return (
                    <li key={o.id} className="rounded-2xl border border-neutral-200 p-3 shadow-sm">
                      <div className="mb-1 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className={`inline-block h-2.5 w-2.5 rounded-full ${statusDot[o.status]}`} title={statusLabel[o.status]} />
                          <a href={`/orders/${o.id}`} className="font-semibold text-indigo-700 hover:underline">
                            #{o.id}
                          </a>
                        </div>
                      ...
                      <div className="text-sm">
                        <div className="truncate"><b>Müşteri:</b> {o.customerName || "—"}</div>
                        <div className="truncate"><b>Telefon:</b> {o.customerPhone || "—"}</div>
                        <div className="truncate"><b>Bayi:</b> {o.dealer?.name || "—"}</div>
        ...
        {/* Panel backdrop (mobil) */}
        {!!selectedYMD && (
          <button className="absolute inset-0 z-20 bg-black/30 lg:hidden" aria-label="Kapat" onClick={() => setSelectedYMD(null)} />
        )}
      </div>
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-40 flex items-end bg-black/40">
          {/* backdrop */}
          <button className="absolute inset-0" onClick={() => setFiltersOpen(false)} aria-label="Kapat" />
          {/* sheet */}
          <div className="relative w-full rounded-t-2xl bg-white p-4 shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-xs text-neutral-600">Arama</label>
                <input
              ...
              <div>
                <label className="mb-1 block text-xs text-neutral-600">Durum</label>
                <select
                  className="h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value as any)}
              ...
              <div>
                <label className="mb-1 block text-xs text-neutral-600">Bayi</label>
                <select
                  className="h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                  value={dealerFilter}
                  onChange={(e) => setDealerFilter(e.target.value)}
